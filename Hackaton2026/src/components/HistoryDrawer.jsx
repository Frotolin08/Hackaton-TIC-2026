  ...
  return (
    <div className="w-full max-w-2xl mx-auto glass-panel p-6 rounded-2xl border border-zinc-800 animate-fade-in flex flex-col gap-6">
      <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
        ...
        <button onClick={onClose} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-bold rounded-xl transition-all border border-zinc-700 cursor-pointer">
          Volver
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin">
          {sets.map((set) => (
            <div key={set.id} onClick={() => onSelectSet(set)} className="flex justify-between items-center p-4 bg-zinc-950/60 border border-zinc-900 rounded-xl hover:border-violet-500/30 transition-all cursor-pointer group">
              <div className="flex flex-col gap-1 min-w-0 pr-4">
                <h4 className="text-sm font-bold text-white group-hover:text-violet-400 transition-colors truncate">{set.title}</h4>
                <div className="flex items-center gap-2 text-[10px] text-zinc-500">
                  <span className="bg-zinc-900 px-2 py-0.5 rounded text-zinc-400 border border-zinc-850">{set.difficulty}</span>
                  <span>|</span>
              ...
              <button onClick={(event) => handleDelete(event, set.id)} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-950/20 rounded-lg transition-all" title="Eliminar set">
                Eliminar
              </button>
            </div>
          ))}
